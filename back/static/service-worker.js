...
const API_CACHE = 'crime-checker-api-v1';
...
/**
 * Listener to intercept and cache api requests, or serve shell.
 */
self.addEventListener('fetch', function (event) {
    event.respondWith(async function () {
        if (event.request.url.indexOf('api') !== -1) { // it is an api hit

            // open the api cache and try and find a hit
            const cache = await caches.open(API_CACHE);
            const response = await cache.match(event.request);
            if (response) return response;

            // fetch from the api itself and cache the result
            else {
